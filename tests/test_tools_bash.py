...
from bladerunner.tools.bash import SUBPROCESS_TIMEOUT, BashTool


def test_bash_tool_executes_simple_command():
...
def test_bash_tool_reports_exit_code_on_failure():
...
def test_bash_tool_handles_nonexistent_command():
...
def test_bash_tool_handles_multiline_output():
...
def test_bash_tool_handles_timeout():
...
def test_bash_tool_handles_empty_output():
...
def test_bash_tool_preserves_exit_status():
...
def test_bash_tool_handles_pipes_and_redirects():
...
def test_bash_tool_handles_command_substitution():
