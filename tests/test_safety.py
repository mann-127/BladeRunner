...
def test_detects_critical_bash_command():
    checker = CriticalOperation()

    is_critical, reason = checker.is_critical_bash("rm -rf /tmp/data")

    assert is_critical is True
    assert reason == "Delete files with 'rm'"


def test_allows_non_critical_bash_command():
    checker = CriticalOperation()
...
def test_detects_sensitive_file_write_paths_and_extensions():
    checker = CriticalOperation()
    ...
    assert path_reason is not None and "sensitive path" in path_reason
    assert is_critical_ext is True
    assert ext_reason is not None and ".pem" in ext_reason


def test_detects_sensitive_reads_only_for_secret_locations():
    checker = CriticalOperation()
    ...
    assert env_reason is not None and "sensitive" in env_reason
