...
def test_session_create_generates_unique_id(tmp_path):
...
def test_session_save_and_load_messages(tmp_path):
...
def test_session_get_last_session(tmp_path):
...
def test_session_persistence_across_instances(tmp_path):
...
def test_session_handles_nonexistent_session(tmp_path):
...
def test_session_message_order_preserved(tmp_path):
...
def test_session_supports_metadata(tmp_path):
...
def test_session_empty_session_returns_empty_list(tmp_path):
...
def test_session_skips_system_context_messages(tmp_path):
    """System context entries should not be written to session history."""
    manager = SessionManager(tmp_path)
    session_id = manager.create_session("no-system")

    manager.save_message(
        session_id,
        {
            "role": "system",
            "content": "[Context:memory]\ntransient memory hint",
        },
    )
