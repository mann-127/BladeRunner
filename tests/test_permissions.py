...
def test_permissive_profile_allows_everything():
...
def test_strict_profile_denies_most_operations():
...
def test_standard_profile_uses_ask_for_sensitive_operations():
...
def test_standard_profile_denies_sensitive_file_reads():
...
def test_standard_profile_bash_command_patterns():
...
def test_non_permissive_profiles_deny_shell_command_chaining():
    """Strict/standard profiles should deny shell control-operator chaining."""
    strict_checker = PermissionChecker(profile="strict")
    standard_checker = PermissionChecker(profile="standard")

    assert strict_checker.check_bash_command("ls -la; rm -rf /") == PermissionLevel.DENY
    assert standard_checker.check_bash_command("echo ok && whoami") == PermissionLevel.DENY
    assert strict_checker.check_bash_command("echo $(whoami)") == PermissionLevel.DENY


def test_permission_patterns_use_glob_matching():
