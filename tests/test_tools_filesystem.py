...
def test_read_tool_reads_file_content(tmp_path):
...
def test_read_tool_handles_file_not_found(tmp_path):
...
def test_read_tool_handles_permission_error(tmp_path):
...
def test_read_tool_handles_utf8_encoding(tmp_path):
...
def test_write_tool_writes_file_content(tmp_path):
...
def test_write_tool_creates_directories(tmp_path):
...
def test_write_tool_overwrites_existing_file(tmp_path):
...
def test_write_tool_handles_utf8_encoding(tmp_path):
...
def test_write_tool_handles_permission_error(tmp_path):
    ...
    if os.name != "nt":
        ...
        tool = WriteTool()
        result = tool.execute(file_path=str(readonly_dir / "file.txt"), content="content")
...
def test_write_tool_definition():
