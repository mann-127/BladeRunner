...
def test_add_and_search_documents(rag_store):
...
def test_search_with_metadata_filtering(rag_store):
    ...
    result_filtered = rag_store.search("web framework", n_results=3, filter_dict={"language": "python"})
...
def test_rag_ingest_tool(tmp_path):
...
def test_empty_documents_handling(rag_store):
